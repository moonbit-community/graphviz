...
PATCH_FILE="$ROOT/tools/graphviz-capture/patches/edge_label_pos.patch"
BUILD_DIR="$GRAPHVIZ_DIR/build"
OUT_DIR="$ROOT/tests/fixtures/graphviz"
OUT_FILE="$OUT_DIR/edge_label_pos.jsonl"
TMP_FILE="$OUT_DIR/edge_label_pos.jsonl.tmp"
...
if command -v rg >/dev/null 2>&1; then
  HAS_MARKER=$(rg -q "MBT_CAPTURE_EDGE_LABEL_POS" "$GRAPHVIZ_DIR/lib/dotgen/dotsplines.c" && echo yes || echo no)
else
  HAS_MARKER=$(grep -q "MBT_CAPTURE_EDGE_LABEL_POS" "$GRAPHVIZ_DIR/lib/dotgen/dotsplines.c" && echo yes || echo no)
fi
...
for input in "$ROOT"/tests/layout/dot/*.dot; do
  ...
  MBT_CAPTURE_EDGE_LABEL_POS="$TMP_FILE" "$DOT_BIN" -Txdot "$input" >/dev/null
done
