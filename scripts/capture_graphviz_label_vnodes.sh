...
PATCH_FILE="$ROOT/tools/graphviz-capture/patches/label_vnode.patch"
BUILD_DIR="$GRAPHVIZ_DIR/build"
OUT_DIR="$ROOT/tests/fixtures/graphviz"
OUT_FILE="$OUT_DIR/label_vnode.jsonl"
TMP_FILE="$OUT_DIR/label_vnode.jsonl.tmp"
...
if command -v rg >/dev/null 2>&1; then
  CLASS2_MARKER=$(rg -q "MBT_CAPTURE_LABEL_VNODES" "$GRAPHVIZ_DIR/lib/dotgen/class2.c" && echo yes || echo no)
  FLAT_MARKER=$(rg -q "MBT_CAPTURE_LABEL_VNODES" "$GRAPHVIZ_DIR/lib/dotgen/flat.c" && echo yes || echo no)
else
  CLASS2_MARKER=$(grep -q "MBT_CAPTURE_LABEL_VNODES" "$GRAPHVIZ_DIR/lib/dotgen/class2.c" && echo yes || echo no)
  FLAT_MARKER=$(grep -q "MBT_CAPTURE_LABEL_VNODES" "$GRAPHVIZ_DIR/lib/dotgen/flat.c" && echo yes || echo no)
fi

if [[ "$CLASS2_MARKER" != "yes" || "$FLAT_MARKER" != "yes" ]]; then
