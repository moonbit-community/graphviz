...
PATCH_FILE="$ROOT/tools/graphviz-capture/patches/record.patch"
...
if command -v rg >/dev/null 2>&1; then
  HAS_MARKER=$(rg -q "MBT_CAPTURE_RECORD" "$GRAPHVIZ_DIR/lib/common/shapes.c" && echo yes || echo no)
else
  HAS_MARKER=$(grep -q "MBT_CAPTURE_RECORD" "$GRAPHVIZ_DIR/lib/common/shapes.c" && echo yes || echo no)
fi
