...
PATCH_FILE="$ROOT/tools/graphviz-capture/patches/xpos.patch"
BUILD_DIR="$GRAPHVIZ_DIR/build"
OUT_DIR="$ROOT/tests/fixtures/graphviz"
OUT_FILE="$OUT_DIR/xpos.jsonl"
TMP_FILE="$OUT_DIR/xpos.jsonl.tmp"
...
if command -v rg >/dev/null 2>&1; then
  HAS_MARKER=$(rg -q "MBT_CAPTURE_XPOS" "$GRAPHVIZ_DIR/lib/dotgen/position.c" && echo yes || echo no)
else
  HAS_MARKER=$(grep -q "MBT_CAPTURE_XPOS" "$GRAPHVIZ_DIR/lib/dotgen/position.c" && echo yes || echo no)
fi
